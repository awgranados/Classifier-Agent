...
class feature_extractor:
    ...
    def bag_of_word_feature(self, sentence):
        ...
        #tokenize vocab word using indeces and values so that we dont need to compute every time
        #then compare to the sentence param
        ...
        counter = Counter(tokenize(sentence)) #dict where the key is the word and the value is it's freq
        rowarr = []
        
        for word in counter:
            if word in self.vocab_dict:
                fMat.append(counter[word])
                rowarr.append(self.vocab_dict[word])
                              
        colarr = [0] * len(rowarr)

        x = sparse.csc_array((fMat, (rowarr,colarr)), shape=(self.d,1)) 
...
class data_processor:
    ...
    def batch_feat_map(self, sentences):
        ...
        if isinstance(sentences, list):
            X = scipy.sparse.hstack([self.feat_map(sentence) for sentence in sentences],format = 'csc')
        else:
...
class classifier_agent():
    ...
    def predict(self, X, RAW_TEXT=False, RETURN_SCORE=False):
        ...
        scoreArr = self.score_function(X)
    ...
    def error(self, X, y, RAW_TEXT=False):
        ...
        predArr = self.predict(X)
        
        errCounter = np.abs(predArr - y)
        
        err = np.mean(errCounter)
        
        #number of false labels that we get are a +1 and then get mean to normalize it
        #and return that as err
    ...
    def loss_function(self, X, y):
        ...
        log_p = (score - np.logaddexp(0,score))
        
        log_1mp = -(np.logaddexp(0,score))
        
        loss = -1 * ((y * log_p) + log_1mp * (1-y))
    ...
    def train_gd(self, train_sentences, train_labels, niter, lr=0.01, RAW_TEXT=True):
        ...
        for i in range(niter):
            ...
            #self.params = self.gradient(, ytrain)
            # TODO =============================================================================
